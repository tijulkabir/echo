...
import com.echo.android.onboarding.BluetoothCheckScreen
import com.echo.android.onboarding.BluetoothStatus
import com.echo.android.onboarding.BluetoothStatusManager
import com.echo.android.onboarding.BatteryOptimizationManager
import com.echo.android.onboarding.BatteryOptimizationPreferenceManager
import com.echo.android.onboarding.BatteryOptimizationScreen
import com.echo.android.onboarding.BatteryOptimizationStatus
import com.echo.android.onboarding.BackgroundLocationPermissionScreen
import com.echo.android.onboarding.InitializationErrorScreen
import com.echo.android.onboarding.InitializingScreen
import com.echo.android.onboarding.LocationCheckScreen
import com.echo.android.onboarding.LocationStatus
import com.echo.android.onboarding.LocationStatusManager
import com.echo.android.onboarding.OnboardingCoordinator
...
class MainActivity : OrientationAwareActivity() {
    ...
    private val forceFinishReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: android.content.Context, intent: android.content.Intent) {
            if (intent.action == com.echo.android.util.AppConstants.UI.ACTION_FORCE_FINISH) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val filter = android.content.IntentFilter(com.echo.android.util.AppConstants.UI.ACTION_FORCE_FINISH)
        if (android.os.Build.VERSION.SDK_INT >= 33) {
            registerReceiver(
                forceFinishReceiver,
                filter,
                com.echo.android.util.AppConstants.UI.PERMISSION_FORCE_FINISH,
                null,
        ...
        com.echo.android.service.AppShutdownCoordinator.cancelPendingShutdown()
        ...
        try { com.echo.android.service.MeshForegroundService.start(applicationContext) } catch (_: Exception) { }
        meshService = com.echo.android.service.MeshServiceHolder.getOrCreate(applicationContext)
        ...
        lifecycleScope.launch {
            ...
            if (permissionManager.isFirstTimeLaunch()) {
                ...
                if (permissionManager.needsBackgroundLocationPermission() &&
                    ...
                    !com.echo.android.onboarding.BackgroundLocationPreferenceManager.isSkipped(this@MainActivity)
                ) {
                    mainViewModel.updateOnboardingState(OnboardingState.BACKGROUND_LOCATION_EXPLANATION)
                } else {
    ...
    private fun handleNotificationIntent(intent: Intent) {
        val shouldOpenPrivateChat = intent.getBooleanExtra(
            com.echo.android.ui.NotificationManager.EXTRA_OPEN_PRIVATE_CHAT, 
            false
        )
        
        val shouldOpenGeohashChat = intent.getBooleanExtra(
            com.echo.android.ui.NotificationManager.EXTRA_OPEN_GEOHASH_CHAT,
            false
        )
        
        when {
            shouldOpenPrivateChat -> {
                val peerID = intent.getStringExtra(com.echo.android.ui.NotificationManager.EXTRA_PEER_ID)
                val senderNickname = intent.getStringExtra(com.echo.android.ui.NotificationManager.EXTRA_SENDER_NICKNAME)
                
                if (peerID != null) {
                    Log.d("MainActivity", "Opening private chat with $senderNickname (peerID: $peerID) from notification")
                    
                    // Navigate to the DM chat screen with this peer
                    chatViewModel.navigateToDMChat(peerID)
            ...
            shouldOpenGeohashChat -> {
                val geohash = intent.getStringExtra(com.echo.android.ui.NotificationManager.EXTRA_GEOHASH)
                ...
                if (geohash != null) {
                    ...
                    val level = when (geohash.length) {
                        7 -> com.echo.android.geohash.GeohashChannelLevel.BLOCK
                        6 -> com.echo.android.geohash.GeohashChannelLevel.NEIGHBORHOOD
                        5 -> com.echo.android.geohash.GeohashChannelLevel.CITY
                        4 -> com.echo.android.geohash.GeohashChannelLevel.PROVINCE
                        2 -> com.echo.android.geohash.GeohashChannelLevel.REGION
                        else -> com.echo.android.geohash.GeohashChannelLevel.CITY // Default fallback
                    }
                    val geohashChannel = com.echo.android.geohash.GeohashChannel(level, geohash)
                    val channelId = com.echo.android.geohash.ChannelID.Location(geohashChannel)
