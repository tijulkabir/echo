...
class NostrTransport(
    ...
    companion object {
        ...
        private const val READ_ACK_INTERVAL = com.echo.android.util.AppConstants.Nostr.READ_ACK_INTERVAL_MS // ~3 per second (0.35s interval like iOS)
        ...
        transportScope.launch {
            try {
                ...
                val recipientPeerIDForEmbed = try {
                    com.echo.android.favorites.FavoritesPersistenceService.shared
    ...
    fun sendPrivateMessageGeohash(
        ...
        val geohash = sourceGeohash ?: run {
            val selected = try {
                com.echo.android.geohash.LocationChannelManager.getInstance(context).selectedChannel.value
            } catch (_: Exception) { null }
            if (selected !is com.echo.android.geohash.ChannelID.Location) {
    ...
    private fun resolveNostrPublicKey(peerID: String): String? {
        try {
            ...
            com.echo.android.favorites.FavoritesPersistenceService.shared.findNostrPubkeyForPeerID(peerID)?.let { return it }

            // 2) Legacy path: resolve by noise public key association
            val noiseKey = hexStringToByteArray(peerID)
            val favoriteStatus = com.echo.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
            ...
            if (peerID.length == 16) {
                val fallbackStatus = com.echo.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
